...
echo "Splitting local FASTQ files with rapidgzip (split every $SPLIT_LINES lines)..."
rapidgzip -d -c -P 0 "$R1_LOCAL_PATH" | split -l $SPLIT_LINES -d -a 4 --additional-suffix=.fastq - "/mnt/nvme/${R1_BASE}_p" &
rapidgzip -d -c -P 0 "$R2_LOCAL_PATH" | split -l $SPLIT_LINES -d -a 4 --additional-suffix=.fastq - "/mnt/nvme/${R2_BASE}_p" &
wait
...
# Delete downloaded, split, and input.txt files after processing
echo "Cleaning up local files..."

# Delete downloaded compressed files
