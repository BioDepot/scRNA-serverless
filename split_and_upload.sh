...
aws s3 cp "$R1_S3_FULL_PATH" - | zcat | split -l $SPLIT_LINES -d --additional-suffix=.fastq - "/mnt/nvme/${R1_BASE}_p" &
aws s3 cp "$R2_S3_FULL_PATH" - | zcat | split -l $SPLIT_LINES -d --additional-suffix=.fastq - "/mnt/nvme/${R2_BASE}_p" &
wait
