...
find /mnt/nvme/ -type f -name "${R1_BASE}_p*.fastq" | while read -r file; do
...
find /mnt/nvme/ -type f -name "${R2_BASE}_p*.fastq" | while read -r file; do
    new_name=$(echo "$file" | sed -E 's/_p0+([0-9])/_p\1/')
    if [[ "$file" != "$new_name" ]]; then
        mv "$file" "$new_name"
    fi
...
find /mnt/nvme/ -type f -name "${R1_BASE}_p*.fastq" | while read -r r1_file; do
